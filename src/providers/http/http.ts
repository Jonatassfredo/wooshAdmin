...
export class HttpProvider {
  ...
  public get(url: string): Promise<HttpResultModel> {
    ...
    return new Promise(resolve => {
      if (this.networkSrv.IsOnline) {
        ...
        // this.alertSrv.toast(
        //   "Você está Offline, e infelizmente não pode ser carregado os dados!",
        //   "bottom"
        // );
      ...
      if (this.networkSrv.IsOnline) {
        this.http.post(url, model, { headers: header }).subscribe(
          ...
          err => {
            ...
            //   "bottom"
            // );
            else resolve({ success: false, data: undefined, err: err });
          }
        );
      } else {
        // this.alertSrv.toast(
  ...
  public put(url: string, model: any): Promise<HttpResultModel> {
    ...
    return new Promise(resolve => {
      if (this.networkSrv.IsOnline) {
        this.http.put(url, model, { headers: header }).subscribe(
          _res => {
            // this.spinnerSrv.Hide();
            resolve({ success: true, data: _res, err: undefined });
          },
  ...
  public delete(url: string): Promise<HttpResultModel> {
    ...
    return new Promise(resolve => {
      if (this.networkSrv.IsOnline) {
        this.http.delete(url, { headers: header }).subscribe(
          _res => {
            // this.spinnerSrv.Hide();
            resolve({ success: true, data: _res, err: undefined });
          },
